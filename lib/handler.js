...
const run = async (router, ctx) => {
	//console.log(ctx.req.complete, ctx.req.socket.writable, ctx.res)
	...
	// forced end
	if (ctx.socket.writable && !ctx.res.writableEnded) {
		ctx.status(404)
		ctx.end()
	}
}
...
module.exports = (server, router) => {
	...
	router.use(ctx => {Object.defineProperties(ctx, {
	...
	router.use(
		...
		require('../middleware/status'),
		require('../middleware/contentType'),
		require('../middleware/contentLength'),
		require('../middleware/cookie'),
		require('../middleware/send'),
		require('../middleware/body')({payload: '8mb'})
	)
